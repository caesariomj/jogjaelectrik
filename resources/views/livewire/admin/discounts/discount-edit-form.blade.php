...
new class extends Component {
    ...
    public function save()
    {
        ...
        try {
            $this->authorize('update', $this->form->discount);

            DB::transaction(function () use ($validated) {
                $this->form->discount->update([
            ...
            session()->flash('success', 'Diskon ' . $validated['name'] . ' berhasil diubah.');
            ...
            Log::error('Database error during discount alteration: ' . $e->getMessage());

            session()->flash(
                'error',
                'Terjadi kesalahan dalam mengubah diskon ' .
                    $this->form->discount->name .
                    ', silakan coba beberapa saat lagi.',
            );
            return $this->redirectIntended(route('admin.discounts.index'), navigate: true);
        } catch (\Exception $e) {
            Log::error('Unexpected discount alteration error: ' . $e->getMessage());
