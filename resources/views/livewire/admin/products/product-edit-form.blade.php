...
new class extends Component {
     ...
     * @return  void
     *
     * @throws  AuthorizationException if the user is not authorized to delete uploaded product image.
    ...
    public function deleteImage(string $id)
    {
        ...
        try {
            ...
            Log::error('Database query error occurred', [
                ...
                'user_id' => auth()->id(),
                'context' => [
                    'operation' => 'Deleting product image data',
     ...
     * @return  void
     *
     * @throws  AuthorizationException if the user is not authorized to update the product.
     * @throws  QueryException if a database query error occurred.
     * @throws  \Exception if an unexpected error occurred.
     */
    public function save()
    {
        ...
        try {
            ...
            Log::error('Database query error occurred', [
                ...
                'user_id' => auth()->id(),
                'context' => [
                    'operation' => 'Updating product data',
                    'component_name' => $this->getName(),
                ],
            ]);
                ...
                @if (count($form->images) > 0 || (! is_null($form->newImages) && count($form->newImages) > 0))
                    ...
                    <ul class="mt-2 grid grid-cols-2 gap-4 sm:grid-cols-6">
                        @foreach ($form->images as $image)
                            <li>
                                <figure class="relative flex h-full w-full flex-col items-center">
                                    <div
                                    ...
                                    >
                                        Gambar produk {{ $form->name . ' - ' . $loop->iteration }}
                                    </figcaption>
                                    <button
                                        type="button"
                            ...
                            @foreach ($form->newImages as $image)
                                <li>
                                    ...
                                    >
                                        ...
                                        >
                                            Gambar produk
                                            {{ $form->name . ' - ' . ($loop->iteration + count($form->images)) }}
                                        </figcaption>
                                        <span
