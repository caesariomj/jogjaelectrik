...
new class extends Component {
    ...
    public function boot(PaymentService $paymentService, DocumentService $documentService): void
    {
    ...
    public function placeholder(): View
    {
        ...
        return view('components.skeleton.order', compact('context'));
    }

    #[Computed]
    public function orders(): LengthAwarePaginator
    {
        $orderIds = Order::queryAllByUserIdAndStatusWithRelations(
            userId: auth()->id(),
            status: $this->status,
            columns: ['orders.id'],
        )
        ...
        $orders = Order::queryAllByUserIdAndStatusWithRelations(
            userId: auth()->id(),
            status: $this->status,
            columns: ['orders.*'],
            ...
            ->map(function ($order) {
                ...
                $orderModel->details = $order->map(function ($detail) {
                    return (object) [
                        ...
                        'subcategory_slug' => $detail->subcategory_slug,
     ...
     * @return  void
     *
     * @throws  AuthorizationException if the user is not authorized to rate the products.
     * @throws  QueryException if a database query error occurred.
     * @throws  \Exception if an unexpected error occurred.
     */
    public function rateProducts(array $data)
    {
        ...
        try {
            ...
            Log::error('Database query error occurred', [
                ...
                'user_id' => auth()->id(),
                'context' => [
                    'operation' => 'Rating order products',
                    'component_name' => $this->getName(),
                ],
            ]);
            ...
            session()->flash(
                'error',
                'Terjadi kesalahan dalam menilai produk-produk dalam pesanan ini, silakan coba beberapa saat lagi.',
            );
            return $this->redirectIntended(route('orders.index'), navigate: true);
        } catch (\Exception $e) {
            Log::error('An unexpected error occurred', [
        ...
        @forelse ($this->orders as $order)
            ...
            >
                <header class="mb-4 border-b border-neutral-300 p-4">
                    ...
                    <div class="absolute end-4 top-4 mb-0.5 inline-flex items-center gap-x-2">
                        <span
                            @class([
                                    ...
                                    <div class="flex w-full flex-col items-start">
                                        ...
                                        >
                                            ...
                                            <optgroup label="Alasan Terkait Pembayaran">
                                                <option value="metode_pembayaran_gagal">Metode Pembayaran Gagal</option>
                            ...
                            >
                                <div class="flex flex-col items-center p-6">
                                    ...
                                    <p class="mb-4 text-center text-base font-medium tracking-tight text-black/70">
                                        ...
                                        menyelesaikan transaksi. Dengan melanjutkan, Anda menyatakan bahwa pesanan telah
                                    ...
                                    <div class="mt-8 flex w-full flex-col justify-end gap-4 md:flex-row">
                                        ...
                                        <x-common.button
                                            ...
                                            wire:target="finishOrder('{{ $order->id }}')"
                                        >
                                            <span wire:loading.remove wire:target="finishOrder('{{ $order->id }}')">
                                                Selesaikan Pesanan
                                            </span>
                                            <span
                                                wire:loading.flex
                                        ...
                                        @foreach ($order->details as $item)
                                            ...
                                            >
                                                <div class="flex items-center gap-x-4">
                                                    ...
                                                    <div class="flex h-20 w-full flex-col items-start justify-center">
                                                        <a
                                                            href="{{ $item->category_slug && $item->subcategory_slug ? route('products.detail', ['category' => $item->category_slug, 'subcategory' => $item->subcategory_slug, 'slug' => $item->slug]) : route('products.detail.without.category.subcategory', ['slug' => $item->slug]) }}"
                                                            class="mb-0.5"
                                                            wire:navigate
